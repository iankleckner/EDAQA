...
%   2019/02/11 Output FILTERED EDA data and option for NO filtering
% 
% LICENSE
...
clc;
clear all;
version_string = '2016.12.18';

%% Input

% Foldername for input data, where inside there are .csv files (and
% potentially other trees of folders with .csv files) that are in the Q
% Sensor format. If you have your own file format then you have to write
% in your own code to load the data, which occurs below
dirname_data_base = 'data';

% Despiking filter (default 2, use NaN to disable it. e.g. you can use your
% own filter beforehand if you like if you write your own code)
...
FIGURE_WIDTH_HTML = 300;

% MATLAB .fig file along with each .png file (slower but better for editing
% later to make publication-quality graphics)
WRITE_DOT_FIG = false;

%% Search directory structure for .eda and .csv files

% Search for files
filename_Q_array = subdir( sprintf('%s/*.csv', dirname_data_base) );
Nfiles = length(filename_Q_array);
...
data_in_array = {};
for f = 1:Nfiles
    filename_Q_full = filename_Q_array(f).name;
    
    % Shot name has just the end of the file (no directories or slashes)
    filename_Q_short = strrep(filename_Q_full, dirname_data_base, '');
    filename_Q_short = strrep(filename_Q_short, '/', '');
    filename_Q_short = strrep(filename_Q_short, '\', '');    
    
    % Save into structure for later loading data
    data_in_array{f}.filename_short = filename_Q_short;
    data_in_array{f}.filename_full = filename_Q_full;    
    
    % Check
    fprintf('\nFile %d/%d: %s', f, Nfiles, filename_Q_short)
end
...
% Foldername for HTML figures
foldername_figures = 'figs';
mkdir(sprintf('%s/%s', foldername_output, foldername_figures));

diary(sprintf('%s/diary.txt', foldername_output));
zip(sprintf('%s/code.zip', foldername_output), {'*.m'});

%--------------------------------------------------------------------------
% HTML file for all PPs

% Open log file for output
filename_output_html_all_PPs = sprintf('%s/index.html', foldername_output);
OUTPUT_HTML = fopen(filename_output_html_all_PPs, 'w');

% HTML header
fprintf(OUTPUT_HTML, '<html>');
fprintf(OUTPUT_HTML, '\n<body>');
fprintf(OUTPUT_HTML, '\n<title>EDAQA</title>');
fprintf(OUTPUT_HTML, '\n<div align="center">');

fprintf(OUTPUT_HTML, '\n<h1>EDA Quality Assessment</h1>');
fprintf(OUTPUT_HTML, '\n\nIan Kleckner, Matthew Goodwin');
fprintf(OUTPUT_HTML, '\n<br />');
fprintf(OUTPUT_HTML, '\nProgram Version %s', version_string);
fprintf(OUTPUT_HTML, '\n<br />');
fprintf(OUTPUT_HTML, '\nRun Time: %s', datestr(now));
fprintf(OUTPUT_HTML, '\n<br />');

fprintf(OUTPUT_HTML, '\n<br />QA Filter Window for EDA: %f sec', QA_filter_window_EDA_sec);
fprintf(OUTPUT_HTML, '\n<br />QA Filter Window for Temperature: %f sec', QA_filter_window_EDA_sec);
fprintf(OUTPUT_HTML, '\n<br />QA_EDA_EFFECTIVE_FLOOR: %f uS', QA_EDA_floor);
fprintf(OUTPUT_HTML, '\n<br />QA_EDA_EFFECTIVE_CEILING: %f uS', QA_EDA_ceiling);
fprintf(OUTPUT_HTML, '\n<br />QA_EDA_MAX_SLOPE_US_PER_SEC: %f uS/sec', QA_EDA_max_slope_uS_per_sec);
fprintf(OUTPUT_HTML, '\n<br />QA_TEMP_MINIMUM: %f C', QA_temperature_C_min);
fprintf(OUTPUT_HTML, '\n<br />QA_TEMP_MAXIMUM: %f C', QA_temperature_C_max);
fprintf(OUTPUT_HTML, '\n<br />RADIUS_TO_SPREAD_INVALID_DATUM_SEC: %f sec', QA_radius_to_spread_invalid_datum_sec);


set(gca, 'LineWidth', LINEWIDTH, 'FontSize', FONTSIZE);

% Convert Unix time to date in whatever time zone
convert_unix_time_to_date = @(time_start_unix, hours_convert_from_UTC) datestr(time_start_unix/86400 + hours_convert_from_UTC/24  + datenum(1970,1,1));
...
PPNum_array                     = [];
PPName_array                    = {};
start_time_YMD_string_array     = {};
day_num_array                   = [];
file_num_array                  = [];

NPPs = length(data_in_array);
for f = 1:Nfiles
    
    % Get info for this participant
    filename_Q_Sensor_short = data_in_array{f}.filename_short;
    filename_Q_Sensor_full = data_in_array{f}.filename_full;
    
    % This is a valid dataset for analysis
    fprintf(OUTPUT_HTML, '\n<hr/>');
    fprintf('\n\nWorking on (%d) %s...', f, filename_Q_Sensor_full);
    ...
    %--------------------------------------------------------------------------
    % Open file
    FILE = fopen(filename_Q_Sensor_full, 'r');
    ...
    k_slashes = strfind(filename_Q_Sensor_full, '/');
    ...
    fprintf(OUTPUT_HTML, '\n<h1>(%d) %s: Started %s</h1>', f, filename_Q_Sensor_short, date_start_Q);
    ...
    column_labels = {'Time(sec)', 'EDA(uS)', 'EDA_Filtered(uS)', 'EDA_Valid(1Yes_0No)', 'EDA_w_NaN_from_QA(uS)', 'Temp(C)'};
    data_matrix = [data_time_sec', data_EDA_uS, data_EDA_uS_filtered, EDA_datum_valid, data_EDA_uS_censored, data_temperature_C]; 
    ...
    title(sprintf('%s (%0.0f%% Invalid)', filename_Q_Sensor_short, 100*QA_invalid_data_fraction));
    xlabel('Time (sec)');
    ylabel('EDA (uS)');    
    legend({'Raw Data', 'Filtered Data', 'Filtered for QA', 'Valid Data'});

    filename_figure = sprintf('out-%03d-1-raw+filter+QA+valid.png', f);
    set(gca, 'LineWidth', LINEWIDTH, 'FontSize', FONTSIZE);
    writePlot([], [], [], filename_figure, foldername_output, foldername_figures, OUTPUT_HTML, FIGURE_WIDTH_HTML, WRITE_DOT_FIG);
    ...
    filename_figure = sprintf('out-%03d-2-valid.png', f);
    set(gca, 'LineWidth', LINEWIDTH, 'FontSize', FONTSIZE);
    writePlot([], [], [], filename_figure, foldername_output, foldername_figures, OUTPUT_HTML, FIGURE_WIDTH_HTML, WRITE_DOT_FIG);
    ...
    filename_figure = sprintf('out-%03d-3-temperature.png', f);
    set(gca, 'LineWidth', LINEWIDTH, 'FontSize', FONTSIZE);
    writePlot([], [], [], filename_figure, foldername_output, foldername_figures, OUTPUT_HTML, FIGURE_WIDTH_HTML, WRITE_DOT_FIG);
    ...
    filename_figure = sprintf('out-%03d-4-acceleration.png', f);
    set(gca, 'LineWidth', LINEWIDTH, 'FontSize', FONTSIZE);
    writePlot([], [], [], filename_figure, foldername_output, foldername_figures, OUTPUT_HTML, FIGURE_WIDTH_HTML, WRITE_DOT_FIG);
    
    %----------------------------------------------------------------------
    
    fprintf(OUTPUT_HTML, '\n\n<br/>Fraction invalid data\t%0.2f%%', 100*QA_invalid_data_fraction);
    
    % Output
    fprintf('Done with this file');    
end
